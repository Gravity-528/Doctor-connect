...
const DoctorSlotCard = ({props}) => {
  ...
  const GetPatient=async()=>{
    try{
    const response=await axios.post("http://localhost:8000/api/v1/doctor/fetchId",{id:props.Patient},{withCredentials:true});
  ...
  return (
    <div className="bg-white shadow-lg rounded-lg p-6 max-w-sm mx-auto mt-6">
      <h2 className="text-2xl font-bold mb-2">Appointment fo {Patient.name} at {props.Time}</h2>
